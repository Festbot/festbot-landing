...
export class DiscoverArtistItem extends Component {
	...
	render() {
		...
		if (isActiveDetails) {
			renderingDetails = <Details artist={artist.slug} />;
		}
		...
		return (
			<div
				...
				style={{
					maxHeight: isActiveDetails ? '1000px' : '120px',
					minHeight: isActiveDetails ? '300px' : '120px',
					backgroundImage: artist.hasPhoto ? 'url(https://chatbot.festbot.com/assets/img/artist/' + artist._id + '.jpg)' : 'none',
			...
			>
				<div id={this.props.artist.name} title={this.props.index} onClick={this.detailsContentOpenHandler} className={classes.listItemWrapper}>
					...
					<div className={classes.detailsIcon}> {!isActiveDetails ? <IconDetails color="white" /> : <IconClose color="white" />}</div>
					<div className={classes.country}>{artist.country}</div>
					<div
						className={classes.details}
						style={{
							maxHeight: isActiveDetails ? '880px' : '0px',
							padding: isActiveDetails ? '0px' : '0px'
						}}
					>
						<div ref={element => (this.activeDetailsDiv = element)}>{renderingDetails}</div>
					</div>
				</div>
			</div>
		);
