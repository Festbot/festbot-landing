			...
			return (
				<li>
					...
					<div className={classes.saveIcon}>
						<StarBorder color={colors.blueGrey300} />
					</div>
				</li>
			);
		});
		...
		return (
			<div>
				...
				<SwipeableViews index={this.state.slideIndex} onChangeIndex={this.handleChange}>
					<div>
						<div className={classes.detailsContent}>
							<div style={{ width: '50%' }}>
								<h2>Start time: </h2>
								<p>
									{moment(this.props.event.startDate).format('hh:mm')}
									<br />
									{moment(this.props.event.startDate).format('MMM Do (dddd)')}
								</p>
							</div>
							<div style={{ width: '50%' }}>
								<h2>End time:</h2>
								<p>
									{moment(this.props.event.endDate).format('hh:mm')}
									<br />
									{moment(this.props.event.endDate).format('MMM Do (dddd)')}
								</p>
							</div>
						</div>
						...
						<div className={classes.detailsContentGenre}>
							{this.state.artist.genres.map((genre, index) => {
								return (
									<div id={index} className={classes.inverse}>
										{genre}
									</div>
								);
							})}
					...
					<div>
						<iframe src={'https://open.spotify.com/embed/artist/' + spotifyId} width="100%" height="300" frameBorder={'0'} seamless allowtransparency="true" />
					</div>
