...
class WebviewFooterMenu extends Component {
	...
	handleItemClick = e => {
		...
		return this.setState({ activeItem: e.props.title });
	...
	render() {
		...
		const tabBuilder = menuItems[viewNameFromRouter].map(item => {
			let iconElement = (
				<FontIcon title={item.name} onClick={this.handleItemClick} className="material-icons">
					{item.iconName}
				</FontIcon>
			);
			...
			return (
				<Tab
					...
					icon={iconElement}
					label={item.name}
					containerElement={!item.route == '' ? <Link to={item.route} /> : <div />}
